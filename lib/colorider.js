	...
	//┘
		...
		function _parse (input, ...values)
		{
			let sheet = this.stylesheet,			
				open = sheet.map((value) => `\u001B[${value.open}m`),
				close = sheet.map((value) => `\u001B[${value.close}m`),

				output = [input, ...values].join(' ').replace(/(\u001B\[(39|49|2[1-57-9])m)/gi, (m) =>
				{
					let i = close.indexOf(m);
					return i < 0 ? m : open[i];
				});

			return `${open.join('')}${output}${close.join('')}`;
		}
